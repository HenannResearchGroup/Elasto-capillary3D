...
!                 |
! 
! This subroutine is for a three-dimensional 10-node tetrahedral 
!  element as shown below with 4pt integration.
!
...
!  *User Element,Nodes=10,Type=U1,Iproperties=1,Properties=5,Coordinates=3,Variables=36,Unsymm
!  1,2,3
!
!     State Variables
!       jj = 0
...
!************************************************************************
     ...
     +  intpt,stat,matflag,face,nNodeSurf,nIntPtSurf,intptSurf
      ...
      parameter(nNodeSurf=6) ! number of nodes on a face, do not change
      parameter(nInt=4)      ! number of integration points, do not change
      parameter(nIntSurf=3)  ! number of surface integration points, do not change
      !
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      !
      real*8 u(nNode,3),du(nNode,ndofel),uOld(nNode,ndofel),
     +  coordsC(mcrd,nNode),Ru(3*nNode,1),Kuu(3*nNode,3*nNode),
     +  Iden(3,3),xi(nInt,3),w(nInt),sh(nNode),dsh(nNode,3),
     +  dshC(nNode,3),dshxi(nNode,3),detMapJ,detMapJC,
     +  F_tau(3,3),detF_tau,T_tau(3,3),Fv_t(3,3),Fv_tau(3,3),
     +  SpTanMod(3,3,3,3),Smat(6,1),Bmat(6,3*nNode),Gmat(9,3*nNode),
     +  Amat(9,9),body(3),BodyForceRes(3*nNode,1),gamma,
     +  Rsurf(3*nNodeSurf,1),Ksurf(3*nNodeSurf,3*nNodeSurf),
     +  coordsCSurf(3*nNodeSurf,1),xiSurf(nIntSurf,2),wSurf(nIntSurf),
     +  shSurf(nNodeSurf),dshxiSurf(nNodeSurf,2),
     +  dshxiMat1(3,3*nNodeSurf),dshxiMat2(3,3*nNodeSurf),E,F,G,H2,H,
     +  dE(1,3*nNodeSurf),dG(1,3*nNodeSurf),dF(1,3*nNodeSurf),
     +  dH(1,3*nNodeSurf)
      !
     ...
     +     three=3.d0,third=1.d0/3.d0)
      ...
      if(nNode.eq.10) then
         if(nInt.eq.4) then
            ...
            write(*,*) 'Incorrect number of int points: nInt.ne.4'
            call xit
         endif
      else
         write(*,*) 'Incorrect number of nodes: nNode.ne.10'
         call xit
      endif
      ...
      if(ndload.gt.0) then
        ...
        do i=1,ndload
          ...
          if(face.eq.1) then
            ...
            ! Obtain integration point local coordinates and weights
	    !
	    if(nIntSurf.eq.3) then
	      call xintTri3pt(xiSurf,wSurf,nIntPtSurf) ! 3-pt integration, nIntPtSurf=3 above
            else
	    ...
	    !
            do intptSurf=1,nIntPtSurf
              ...
              ! Obtain shape functions and their local gradients
              !
              call calcShapeTriQuad(nIntPtSurf,xiSurf,intptSurf,
     +                                 shSurf,dshxiSurf)
...
!************************************************************************
      ...
      real*8 xi_int(nIntPt,2),sh(6),dshxi(6,2),xi,eta,lam
      !
